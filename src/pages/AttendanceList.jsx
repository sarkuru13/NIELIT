...
function AttendanceList() {
  ...
  const studentsForBulkAdd = useMemo(() => {
    ...
    if (bulkSemester) studentsInCourse = studentsInCourse.filter(s => s.Semester === parseInt(bulkSemester));
  ...
  const availableSemestersForExport = useMemo(() => {
    ...
    const studentsInCourse = students.filter(s => s.Course?.$id === exportCourseId);
    return Array.from(new Set(studentsInCourse.map(s => s.Semester).filter(Boolean))).sort((a, b) => a - b);
  ...
  const handleExport = () => {
    ...
    if (exportType === 'dateRange') {
        ...
        let filteredStudents = students.filter(s => s.Course?.$id === exportCourseId);
